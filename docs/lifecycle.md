...
Player::init():
    ...
    // 唯一职责: IoC 装配（其中启动 ApiLayer 命令线程）
...
```
Uninitialized ──init()──▶ Idle (模块体系已装配, ApiLayer 命令线程运行, 无媒体)
               ◀──────────
                shutdown()
```
