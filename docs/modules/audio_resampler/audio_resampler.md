...
> 音频重采样模块。位于 AudioDecoder 与 AudioSink 之间，把解码原始 PCM 转成 cpal 输出目标格式。
> 对外接口由 ApiLayer 调用（见 `docs/modules/api_layer/` 各命令编排）。本文件描述其内部设计。

## Context

当前音频链路被悄悄假设了一个前提：open.md 里 `audio_sink.setup(demuxer.audio_config)` 用**解码侧格式**直接建 cpal 流。但 FFmpeg 解出的 PCM 格式（采样率/声道/采样格式/planar↔packed）由**流的编码**决定，cpal 能输出的格式由**平台/设备**决定，**两者未必一致**（如流是 44100/s16/5.1，cpal 只支持 48000/f32/stereo）。

AudioResampler 填这个 gap：把解码原始 PCM 转成 cpal 能吃的格式。顺带解放"AudioSink 必须支持解码格式"这个不成立的前提——cpal 按自己支持的格式建流，重采样负责对齐。
...
AudioPacketQueue(gen) →[AudioDecoder]→ AudioFrameStore(gen, 解码原始PCM)
                                        →[AudioResampler]→ AudioResampledStore(gen, cpal目标格式PCM) →[AudioSink]→ 声卡
```
...
- **格式转换**：解码原始 PCM（采样率 / 声道数 / 采样格式 / planar↔packed）→ cpal 输出目标格式。底层用 FFmpeg `swr_convert`。
...
- ❌ 不解码（AudioDecoder 的事）。
- ❌ 不输出到声卡（AudioSink 的事）。
- ❌ 不决定目标格式（由 cpal/设备能力决定，open 时协商一次传给它，见 configure）。
...
**为什么不在 cpal 实时线程**：重采样是**有状态的计算**（滤波器历史），`swr_convert` 内部会分配缓冲。cpal 实时线程要求零阻塞、零 malloc，放进去是反模式。符合现有架构哲学：**每个工作模块一线程、靠 Store 解耦、cpal 实时线程只做 try 取 + 喂声卡（零计算）**。
...
- 目标格式 config（采样率 / 声道 / 采样格式）——来自 AudioSink 探测 cpal 设备能力后产出的 `audio_output_config`。
- 与 open.md 里"decoder 用 configure(config) 自建"是**同一模式**：ApiLayer 协调，AudioSink 探测产出 output_config，`audio_resampler.configure(input=解码格式, output=cpal格式)`。
...
1. **模块清单**：工作模块层 +1 `AudioResampler`（1 个 loop 线程）；资源管理者层 +1 `AudioResampledStore`（无锁 SPSC `rtrb`，每块带 generation，与现有 Store 同模式）。
2. **数据流图**：音频链路从 3 段变 4 段（多 Resampler 一跳）。
3. **open 编排**（open.md）：`audio_sink.setup` 改为"探测 cpal 能力 → 产出 output_config"；新增 `audio_resampler.configure(input, output)`。
...
- **不塞进 AudioSink**：AudioSink 复用 cpal 实时线程，重采样是有状态计算 + 会 malloc，放进去违反实时约束。
- **不塞进 AudioDecoder**：重采样的目标格式由 cpal 能力决定（输出侧），而 decoder 只懂流的编码（输入侧）。把输出侧知识塞进 decoder，破坏"decoder 只面对一份配置 + 队列"的自包含原则。
- **独立模块 + 独立 Store**：每个工作模块产出一个 Store，模式统一、可单测（构造 Resampler 不需先构造 cpal/Sink）。

### 目标格式 open 时定，非运行时协商
目标格式是静态配置（cpal 设备能力在 open 时探测一次）。Resampler configure 一次后运行时只做转换，不查格式。这让 Resampler 不运行时依赖 AudioSink，DAG 干净。
...
- ❌ open 时 cpal 能力探测的细节 → audio_sink.md（待设计）
