...
```
              写(校准基准)                读(推算当前 pts)
AudioSink(cpal 实时线程) ──→ AudioClock ←── VideoSync / ProgressReporter
                                    ↑
...
- **不做**:不决定播什么(VideoSync 选帧)、不驱动音频播放(cpal)、不参与世代号(它是标量不是数据流)。
...
- **被 AudioSink 校准**:cpal 回调喂声卡数据后,用样本计数算出 ground truth 当前 pts,刷新基准(详见校准机制)。
...
时钟分两层。校准层只在 cpal 回调里跑,算 ground truth;推算层给查询者,在两次校准间用 wall clock 插值。每次校准 = 用校准层结果刷新推算层基准。

### 校准层(回调里,精确,ground truth)

cpal 回调每次喂声卡 N 个样本后,用**绝对值**校准当前 pts:

```
当前 pts = first_pts + (provided − buffered) / sample_rate
                    ↑           ↑
              已提供给 cpal    声卡 buffer 残留
              的样本总数       (还没真播出去的)
```

- `first_pts`:首帧音频的 pts(**取自第一块喂声卡的 PCM 块的 pts,不是算出来的**)。整个会话不变,直到 reset/jump_to 重置。详见"首帧 pts 的确立"。
- `provided`:累计已提供给 cpal 的样本数。
...
```
cpal 回调(喂一块 PCM,带 block_pts + N 样本):
...
无线程,被多线程读写:
- **写者**:AudioSink(cpal 实时线程,校准基准);ApiLoop(freeze/unfreeze/jump_to/reset)。
- **读者**:VideoSync、ProgressReporter(各自工作线程)。

**原子性**:`(base_time, base_pts, rate, frozen, frozen_pts)` 是复合状态,多线程读需一致快照。用 `arc-swap` 整体替换指针——读端无锁、写端罕见(校准/冻结/跳点)。与 api_layer.md 里 MediaInfo 的方案一致。
...
- play 启动出声:cpal 第一次回调校准 → frozen=false,base_time/base_pts 确立。
...
| `calibrate(block_pts, samples_this_call, buffered_samples)` | AudioSink cpal 回调 | 首次确立 first_pts(取 block_pts);算 ground truth pts,刷新 base_time/base_pts;若 frozen 则 unfreeze |
...
> 接口签名细节(参数类型、arc-swap 怎么用)归实现阶段。
...
### arc-swap 原子快照
复合状态(base_time/base_pts/rate/frozen/frozen_pts)多线程读需一致。arc-swap 整体替换指针,读端无锁、写端罕见。不拆成多个 AtomicXX(会撕裂)。
...
### 声卡 latency 的获取
`buffered`(声卡 buffer 残留样本数)需通过 cpal latency API(`stream.latency()`,平台支持时)或自维护 ring 计数(送多少 vs 估计播了多少)获取。平台差异在实现阶段处理,不影响架构。
...
- seek 后音频链路重新定位+解码+重采样需要时间,这期间 cpal 回调还在跑,buffer 里可能还有**旧世代数据**。
...
### cpal 回调里的开销
calibrate 在 cpal 实时线程执行,必须极轻量(算术 + arc-swap store),不拿重锁、不干重活、不 malloc。arc-swap store 是无锁原子操作,符合实时约束。
...
- ❌ cpal latency API 的具体使用 / 自维护 ring 计数的实现 → 实现阶段
- ❌ arc-swap 的具体用法 / 状态结构体的字段布局 → 实现阶段
