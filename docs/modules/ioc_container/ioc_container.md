...
| 方案 | 适合 | 本项目 |
|------|------|--------|
| 自动注入框架(注册+反射/动态类型查找) | 模块多、依赖动态、跨 crate 插件 | ❌ 过度设计 |
| **手动装配(按拓扑顺序构造 + 注入 shared_ptr)** | 模块数可控、依赖静态已知 | ✅ 28 个模块,DAG 已定 |

手动装配的好处:① 编译期就能查依赖(C++ 类型系统帮你验证——传错 shared_ptr 类型编译不过);② 装配顺序肉眼可见,不用追框架;③ 无运行时开销;④ 清晰直白比花哨框架更显功底。
...
```
Player::init()  ──→  IoCContainer::assemble()  ──→  按 DAG 拓扑构造所有模块 + 注入 std::shared_ptr<依赖>
                                                              │
                                                              ▼
                                                  装配完成,持有所有模块 shared_ptr
                                                  ApiLayer/ApiLoop 拿到所需 shared_ptr 自持
                                                              │
                                              [运行时: IoC 透明,只保活,不被访问]
                                                              │
...
1. **装配(assemble)**:按 DAG 拓扑顺序逐个构造模块,构造时注入 `std::shared_ptr<依赖>`,最终持有所有模块的 shared_ptr。装配后把工作模块 shared_ptr 分发给 ApiLayer/ApiLoop(它们自持,运行时不再访问 IoC)。
2. **释放(dispose)**:**手动逆序**释放所有模块(被依赖者后释放),让各模块析构自管资源回收。释放前可插入 stop 逻辑(如先停 ApiLoop、停工作线程再 reset)。

**不做的**(明确边界,防过度设计):
- ❌ 不提供运行时服务定位(不按类型查找模块)——装配后 shared_ptr 直接持有,不用查。
- ❌ 不做自动依赖注入——依赖关系写在每个模块 `constructor` 签名里,装配代码手动传。
...
当前 `IoCContainer::assemble()` 已经装配音频播放和 CPU 视频渲染链路：
...
FfmpegDemuxerBackend        -> DefaultDemuxer
...
FfmpegVideoRendererBackend  -> DefaultVideoRenderer -> DiscardingVideoRenderedSink

ApiLayer(demuxer, video_decoder, video_renderer,
         audio_decoder, audio_resampler, audio_output)
```

`VideoRenderer` 当前将解码帧经 FFmpeg `sws_scale` 转成 RGBA8，
输出接 IoC 内部的 `DiscardingVideoRenderedSink`，消费后直接释放。
这是临时末端，目的是先验证 `VideoDecoder -> VideoRenderer` 链路，
避免尚未接入宿主呈现时因 bounded `VideoRenderedStore` 无消费者而形成背压。
宿主帧交付和真正的 `VideoRenderedStore` 消费者属于后续阶段。
...
```cpp
class IoCContainer {
    // 持有所有模块的 std::shared_ptr (装配后存在,dispose 后清空)
    // 成员声明顺序 = 装配顺序(第0层先声明,接口层后声明)
    // 注意:dispose 是手动逆序析构,不依赖成员声明顺序(见下)
    std::shared_ptr<Generation> generation;
    std::shared_ptr<Notifier> notifier;
    std::shared_ptr<GpuDevice> gpu_device;          // GpuDevice 抽象基类, 按平台选派生类
    ...
    // ... 各工作模块
    std::shared_ptr<ApiLoop> api_loop;
    std::shared_ptr<ApiLayer> api_layer;

public:
    /// 装配:按 DAG 拓扑顺序构造所有模块,注入依赖
    // 无参——装配是确定性的(模块清单+依赖固定),平台差异(#ifdef)在内部处理
    static IoCContainer assemble() {
        ...
        auto demuxer = std::make_shared<Demuxer>(
            video_packet_queue, audio_packet_queue, subtitle_packet_queue,
            generation, notifier
        );
        auto video_decoder = std::make_shared<FfmpegVideoDecoder>(
            video_packet_queue, video_frame_store,
            generation, gpu_device, notifier
        );
        // ... 各 decoder / resampler
        ...
        auto api_layer = std::make_shared<ApiLayer>(api_loop, /* 各工作模块 shared_ptr */);

        return { generation, notifier, gpu_device, /* ... */ api_loop, api_layer };
    }

    // 释放:手动逆序 reset(先停依赖者,后释放被依赖者)
    // 显式写释放顺序,不靠成员声明顺序的隐式约定
    void dispose() {
        // 先停接口层(它持有工作模块 shared_ptr,先 reset 才能让工作模块 shared_ptr 计数归零)
        api_layer.reset();
        api_loop.reset();
...
### 手动装配,不用自动注入框架
28 个模块、依赖静态已知、DAG 已定,手动装配够用。自动注入框架(注册+查找)是为"模块多/动态/插件化"设计的,本项目用不上,引入只增加复杂度。手动装配让依赖关系写在装配代码里,一眼看清,编译期类型系统验证(传错 shared_ptr 类型编译不过)。

### dispose 手动逆序释放,不靠 C++ 自动析构
C++ 成员按声明顺序逆序自动析构,看似能自动实现逆序释放,但有**静默的正确性风险**:
- shared_ptr 是引用计数,模块真正析构 要等所有 shared_ptr copy 都没了。
- IoC 持有各模块 shared_ptr,ApiLayer 也持有工作模块 shared_ptr 的 copy。
- 若靠字段声明顺序,一旦顺序写错(被依赖的字段声明在依赖者之后),编译器不报错,运行时可能 use-after-free(被依赖者先没了,依赖者还在用)。

**手动逆序 reset 的优势**:
- **意图显式**:释放顺序写在代码里,一眼看清,不靠隐式约定。
- **顺序可控**:不依赖 C++ 自动析构 规则,想怎么排就怎么排。
- **可加释放逻辑**:某模块释放前要"优雅停止"(如先停线程再 reset),手动释放能在 reset 前插入模块专属的 shutdown 调用；工作模块的 worker 线程由模块析构自管。
- **配合 lifecycle.md**:lifecycle.md 说"ApiLoop 退出 → IoC 逆序释放模块",手动释放正好对应这个编排。

**释放顺序关键**:ApiLayer/ApiLoop 最先释放(它们持有工作模块 shared_ptr,先释放才能让工作模块 shared_ptr 计数归零,工作模块才能真正析构)。
...
并发装配的代价:
- 复杂度大增(并发构造 + shared_ptr 注入,要处理线程安全、JoinHandle、错误传播)。
...
GpuDevice 的 `constructor` 可做成轻量(只记配置,不立即建 D3D11 device),真正建 device 推迟到第一次 `acquire_buffer`/`device_handle` 调用时(懒加载)。这样:
- 装配期所有模块都微秒级,串行也无感。
- D3D11 device 建在第一次硬解需要时,符合"按需初始化"。
- dispose 时 GpuDevice 析构自动释放 device。
- 串行装配更无可挑剔——连唯一瓶颈都消除了。

> 这是 GpuDevice 实现细节,归 GpuDevice 文档/实现阶段。IoC 这里只受益:装配期无重模块。

### 装配后 shared_ptr 分发自持,运行时不访问 IoC
装配时 ApiLayer/ApiLoop 就拿到所需模块 shared_ptr 的 copy 并自持。运行时调命令直接用自持的 shared_ptr,不经过 IoC 查找。IoC 的唯一运行时作用是"持有所有 shared_ptr 让模块保活直到 dispose"。这符合现有 DI 设计(构造期注入,运行时直接持有,不查容器),IoC 运行时透明。
...
- **architecture.md**:"init 时按 DAG 拓扑顺序构造所有模块、构造时注入 std::shared_ptr<依赖>;shutdown 时逆序释放"——职责对得上。
- **各模块 `constructor` 签名**:暴露依赖(显式可见),装配代码把这些串起来。依赖关系在 `constructor` 签名里,装配代码是"把这些签名串起来"的胶水。

---

## 坑与边界

### dispose 顺序与 shared_ptr 引用计数
shared_ptr 模块真正析构 要等所有 shared_ptr copy 都没了。dispose 手动逆序 reset IoC 持有的 shared_ptr,但 ApiLayer 等也持有 copy——必须先 reset ApiLayer(它的 copy 没了),工作模块的 shared_ptr 计数才能归零真正析构。所以 dispose 顺序里 ApiLayer/ApiLoop 必须最先。这是手动逆序释放的核心约束。

### 装配失败的错误处理
某模块构造失败(如 GpuDevice 建 device 失败)怎么办?assemble 返回 `Result<IoCContainer, Error>`,失败时已构造的模块要回滚(逆序 reset 已建的部分)。这是实现细节,文档标"装配失败需回滚已构造模块"。

### 平台差异(#ifdef platform macro)
GpuDevice 的具体实现按平台 #ifdef platform macro 选(Windows → D3D11GpuDevice)。assemble 内部用 #ifdef platform macro 分支,对外接口不变(都返回 `std::shared_ptr<dyn GpuDevice>`)。其它模块无平台差异。

### 字段声明顺序
虽然 dispose 是手动逆序(不依赖成员声明顺序),但成员声明建议按装配顺序(第0层先),便于阅读和与 assemble 代码对应。这不影响释放(手动 reset 顺序由 dispose 函数决定)。

---

## 边界（本文档不涉及）

- ❌ 各模块 `constructor` 的具体签名 → 各模块文档
- ❌ 装配失败的具体回滚实现 → 实现阶段
