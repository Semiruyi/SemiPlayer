...
| 视频/音频流索引 | open 探测后确定 |
| 线程状态（见状态机）| 运行时 |
| `seek_intent: std::mutex + std::optional<(pos, std::promise<void>)>` | seek 命令设置，工作线程消费 |
...
#### `seek(pos)` — std::promise 同步等待
```
seek(pos):
    std::promise<void> tx; auto rx = tx.get_future()
    { std::lock_guard lk(seek_intent_mu); seek_intent = std::make_optional(std::pair{pos, std::move(tx)}); }  // 存意图 + 完成信号
    cv.notify_one()                           // 唤醒工作线程(若在队列满wait上)
    rx.get()                                  // 等工作线程处理完定位
    // 返回 (定位已完成)
```
- 符合 seek 命令"完成才 resolve"。
- std::promise 天然防 lost wakeup（set_value 早于 get 也保留）。
- **连续 seek**（前一个未处理）：新 seek 覆盖 seek_intent，旧 promise 析构 → 旧 seek 的 `rx.get()` 抛 `std::future_errc::broken_promise` → 旧 seek 返回"被取消"。实践中 ApiLayer 串行 + Dart 侧 cancel 旧 handle，少触发，但内部 robust 处理。
...
void run_loop():
    loop:                                              # ← 循环顶, 所有唤醒回到这里
        ...
        while not 该干活(g):
            cv.wait(lk)                                # 醒来重新检查, 不判断唤醒来源
        
        switch (state):
            case Stopping:
                state = Stopped; return                # 唯一退出点 (lk RAII 释放)
            
            case SeekIntent{pos, tx}:
                state = Seeking;                        // lk 作用域结束释放
                av_seek_frame(pos)
                generation.fetch_add(1)                # 定位后+1, 与读新数据绑定
                clear seek_intent
                tx.set_value()                          # 告诉 seek(): 定位完成
                state = Reading
                continue                               # 回顶读新
...
- ApiLoop 调 seek（std::promise tx + notify cv）。
...
### seek 用 std::promise channel 同步等待
std::promise 天然是"请求-响应"语义，防 lost wakeup。seek 设 SeekIntent+promise、notify、等 future。连续 seek 旧 promise 析构 → 旧 seek 收 broken_promise（取消），与 cancel 语义一致。
...
### std::promise 连续 seek
连续 seek 时旧 promise 被析构，旧 seek 的 future 收 broken_promise。内部要处理异常（返回取消），不能抛出。
