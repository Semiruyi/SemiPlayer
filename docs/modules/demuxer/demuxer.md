...
| `Notifier` | 订阅 QueueNotFull（队列满时被唤醒）；发布 EOF / Error |
...
Constructed ─open()─▶ Idle ─start_reading()─▶ Reading ⇄ Seeking
                        ▲                          │
                        │   EOF (发 EOF 事件, 线程 wait 在 Idle, 不退出)
                        │
    ...
    loop:                                              # ← 循环顶, 所有唤醒回到这里
        ...
        match state:
            ...
            Reading =>
                ...
                match pkt:
                    EOF =>
                        notifier.publish(EOF)          # → ApiLayer 设 player_state=Ended
                        state = Idle                   # 回 Idle, 线程 wait(不退出)
                        continue                       # 回顶 wait
                    Error(e) =>
                        notifier.publish(Error(e))
                        state = Idle; continue
                    Ok(pkt) =>
...
try_push(queue, pkt) -> bool:        # 返回是否成功放入
    ...
    queue.push(pkt)
    notifier.publish(QueueNotEmpty)   # 通知 decoder 可取包
    return true
```
...
- **QueueNotFull（VideoPacketQueue / AudioPacketQueue）**：队列从满变非满时，Notifier 回调唤醒 demuxer 的 cv。工作线程醒来后**回到循环顶**重新检查（不判断被谁唤醒），发现队列不满 + state=Reading → 继续 try_push。
  - 回调在**发布方线程**（decoder pop 包的线程）执行——必须极轻量（仅 `cv.notify_one()`），不拿重锁、不干重活。

### 发布（通知别人）
- **EOF**：读到文件结尾 → publish(EOF) → ApiLayer 订阅，设 player_state=Ended。
- **Error**：解封装出错 → publish(Error) → ApiLayer 处理。
- **QueueNotEmpty**：push 包后队列非空 → publish → decoder 订阅，唤醒它取包。
...
### EOF 后回 Idle，线程不退出
EOF 发事件后 state=Idle，线程 wait（方案 Y）。可再 seek（调起点重播）或 close。ApiLayer 收 EOF 设 player_state=Ended。
...
- ❌ ApiLayer 如何订阅 EOF/Error → api_layer.md（待补）
