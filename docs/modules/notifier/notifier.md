...
- `T` 是通知类型（C++ 里是个 struct，**业务模块定义**，见下）。
- 内部按 `std::type_index`（T 的类型标识）维护 `HashMap<std::type_index, Vec<回调>>`。
- `send` 时按 T 的 std::type_index 找注册列表，逐个调用回调。
...
struct VideoQueueNotEmpty {};
struct VideoQueueNotFull {};
...
```cpp
class Notifier {
    // std::type_index -> 该通知类型的回调列表 (异构存储, 按 std::type_index 取回)
    std::mutex mu;
    std::unordered_map<std::type_index, std::any> slots;
public:
    // 注册通知: T 类型通知发生时调 cb。cb 接收 const T&, 可读通知数据。
    template <class T>
    void register_cb(std::function<void(const T&)> cb);
...
（内部用 `std::any` 存异构回调列表、按 `std::type_index` 取回，是 C++ 类型化通知中心的标准实现模式。）
...
### 一个全局实例
所有通知类型流经同一个 Notifier，按 std::type_index 分流。shared_ptr 注入给所有模块。不碎片化（无需每模块一个 Notifier）。
