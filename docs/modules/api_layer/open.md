...
③ 步的 `configure` / `setup` 只建上下文（解码器、cpal 流），**不启动工作线程、不开始读数据**。线程启动和数据流动是 play 的职责。open 后管道是"冷的"——已装配、未运行。
...
### cpal 流 setup 但不 start
audio_sink 在 open 时按媒体 PCM 格式建好 cpal 输出流，但处于暂停态（不出声）。等 play 才 `start_playback`。避免 open 完还没 play 就出声。
