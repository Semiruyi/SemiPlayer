...
> 跨平台在 IoC 装配期解决:按平台选具体实现(D3D11GpuDevice / VulkanGpuDevice / ...)。消费者拿到的是 `Arc<dyn GpuDevice>` 契约,不知背后是什么 API。
...
- ❌ 硬解/视频帧语义(不知业务)。
- ❌ shader/pipeline/texture 跨上下文共享(直通路能力,本阶段不实现,trait 不留这些方法)。
...
| `acquire_buffer()` | `GpuBufferGuard`(RAII) | 分配一块 GPU 内存,drop 时自动归还内部池 |
...
**MVP 只实现 D3D11GpuDevice**,但 trait + 枚举已含其他 API 位。未来加后端只加实现,不改 trait/消费者核心逻辑。

---

## 消费者如何用(以 FfmpegVideoDecoder 为例)

GpuDevice 不被 ApiLoop 直接调(它是资源不是工作模块)。VideoDecoder(实际是 FfmpegVideoDecoder 实现)configure 时从注入的 `Arc<dyn GpuDevice>` 取设备能力:

```rust
FfmpegVideoDecoder::configure(config, gpu: Arc<dyn GpuDevice>):
    // 1. 识别 API 类型
    match gpu.api_type() {
        D3D11  => 用 gpu.device_handle() 的 D3D11 句柄构造 AVHWDeviceContext<D3D11VA>
        Vulkan => 用 Vulkan 句柄构造 AVHWDeviceContext<Vulkan>
        ...    // 各 API 的 FFmpeg 桥接分支
    }
...
### VideoDecoder 的 trait + 具体实现(通用逻辑可换后端)

```
trait VideoDecoder {                    // 通用逻辑契约
    fn configure(config, gpu: Arc<dyn GpuDevice>)
    fn start() / stop() / seek()
    fn decode_loop()                    // 取包→解码→喂 Store 的通用骨架
}
...
通用骨架(decode_loop、世代号检查、喂 Store)在 trait/共享代码里,FFmpeg 桥接是 FfmpegVideoDecoder 内部的 match 段。换解码后端(如 AVFoundation/VideoToolbox)→ 写新 trait 实现,通用骨架复用。
...
- **构造期注入给**:copy-back 路下仅 `FfmpegVideoDecoder`(通过 VideoDecoder trait 注入 `Arc<dyn GpuDevice>`)。
- **自身依赖**:无(不依赖任何播放器模块,不依赖 FFmpeg)。
- **平台后端**:IoC 装配期按 cfg 选(MVP: D3D11GpuDevice)。
...
### 为什么 MVP 只 D3D11 后端
跨平台 GPU 抽象的复杂度大多在"胶水"(D3D11/Vulkan/Metal 各一套 + 跨平台纹理共享),对"音视频理解"目标 ROI 低。D3D11 单后端能跑硬解 + 出画面,够展示能力。多后端留 trait/枚举扩展点,面试时讲"MVP 选单后端 D3D11,trait 留多 API 扩展,因为跨平台 GPU 胶水 ROI 低"——体现取舍判断。
...
各后端实现(D3D11GpuDevice/VulkanGpuDevice/...)新增对应能力。trait 扩充,消费者按需调用。
