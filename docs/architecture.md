...
|------|--------------|---------|
| Notifier | 无（被所有人依赖）| — |
| Demuxer | VideoPacketQueue, AudioPacketQueue, SubtitlePacketQueue, Generation, Notifier | ApiLayer 命令线程调 open()/start_reading()/seek()/stop_reading()/close()；阻塞时在自己的 cv 上等，Notifier 回调唤醒（QueueNotFull/Seek/Stop 等）|
