...
- **命令队列 + 句柄（对外控制模型）**：Dart 调用 → 往 ApiLayer 命令队列投递命令 → 立即返回句柄（UI 永不阻塞）。ApiLoop 单线程串行执行命令。句柄可 await（拿结果/确认）可 cancel。并发/节奏/时延由 Dart 业务侧自治，C++ 忠实串行执行 + 提供机制。控制信号走命令队列，不靠通知中心分发
...
| AudioResampler | AudioFrameStore, AudioResampledStore, Generation, Notifier | ApiLoop 调 configure()/start()/stop()/seek()；取 AudioFrameStore 经 swr_convert 转 miniaudio 输出格式喂 AudioResampledStore；查 generation 自 flush；Notifier 唤醒（FrameReady/NotFull 等）|
...
| AudioSink | AudioResampledStore, AudioClock, Generation, miniaudio | ApiLoop 调 setup()/start_playback()/stop_playback()/set_volume()；复用 miniaudio 实时线程（回调驱动，不需 Notifier）|
| ProgressReporter | AudioClock, StreamSink | 独立线程，读 AudioClock 推进度 |
| CommandQueue | 无 | — |
| Generation | 无 | — |
| AudioClock | 无（被 AudioSink 写 / VideoSync·ProgressReporter 读 / ApiLoop 控制） | ApiLoop 调 reset()/freeze()/unfreeze()/jump_to();AudioSink 回调调 calibrate();读端无锁 current_pts() |
| ApiLoop | CommandQueue + 各工作模块 `std::shared_ptr` | — |
...
1. **去中心化状态**：每个工作/资源模块有自己的 `enum class State`（如 `Running/Paused`），无全局上帝状态。ApiLayer 的 `PlayerState` 快照仅是给 Dart 查的投影，不参与控制。
